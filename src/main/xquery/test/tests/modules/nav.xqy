...
module namespace t = "http://missionary.lds.org/audience/cutlass/test/tests/modules/nav";

import module namespace nav = "http://missionary.lds.org/audience/cutlass/modules/nav" at "/modules/nav.xqy";
...
declare function (:TEST:) isolateChannels_noUrlNoChannels() {
  let $url := ()
  let $channels := ()
  let $actual := nav:isolateChannels($url, $channels)
